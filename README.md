# MySQL DBaaS Market Comparison: AWS RDS for PostgreSQL - MS Azure Database for MySQL Flexible Server - OTC RDS MySQL 
...
A Leading DBaaS](TODO) on comparing the performance and price-performance of AWS RDS for MySQL, MS Azure Database for MySQL - Flexible Server and OTC RDS MySQL. For more details on the method and the benchmark objectives please refer to the associated article. 
...
The `0_tpcc_*.csv` and `0_twitter_*.csv` files contain fine grained performance metrics on a per query basis. 
